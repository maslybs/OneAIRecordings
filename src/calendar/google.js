...
import { APP_DIR, CONFIG_PATH, log, readJson, safeName, writeJson } from '../runtime/common.js';
...
export function extractMeetUrl(event) {
  ...
  for (const entryPoint of event.conferenceData?.entryPoints || []) {
    if (entryPoint.uri && MEET_RE.test(entryPoint.uri)) return entryPoint.uri.match(MEET_RE)[0];
  }
  const hay = [event.location, event.description, event.summary].filter(Boolean).join('\n');
  const match = hay.match(MEET_RE);
...
export async function syncCalendarJobs(cfg = readJson(CONFIG_PATH, {})) {
  ...
  cfg.jobs ||= [];
  const existing = new Set(cfg.jobs.map(job => job.id));
  let added = 0;
  let skipped = 0;
  for (const event of events) {
    if (!shouldRecordEvent(event, calendarCfg)) {
      skipped++;
      continue;
    }
    const job = eventToJob(event, calendarCfg);
    if (!job || existing.has(job.id)) {
      skipped++;
      continue;
    }
    cfg.jobs.push(job);
